...
GT: original (flac)
LR: Codec output (wav)
Target: LR + GT with HPF (wav)
Only Focuses on Restoring High Frequencies
"""
class HighTargetDataset(Dataset):
    ## GT : flac
    ## Else : wav (codec)
    ## Target : 
    def __init__(self, gt_path, target_path, lr_path, 
                 seg_len=2, iscodec=True, mode="train"):
        ...
        paths_gt = get_audio_paths(self.gt_path, file_extensions='.flac')
        paths_target = get_audio_paths(self.target_path, file_extensions='.wav')
        paths_lr = get_audio_paths(self.lr_path, file_extensions='.wav')

        print(f"{mode}: GT {len(paths_gt)} / Target {len(paths_target)} / LR {len(paths_lr)} file numbers loaded!")

        if (len(paths_gt) != len(paths_target)) or (len(paths_target) != len(paths_lr)) :
            sys.exit(f"GT {len(paths_gt)} and Target {len(paths_target)} LR {len(paths_lr)} file numbers different!")

        self.filenames = [(path_gt, path_target, path_lr) for path_gt, path_target, path_lr in zip(paths_gt, paths_target, paths_lr)]
        # print(f"{mode}: {len(self.filenames)} files loaded")
    ...
    def __getitem__(self, idx):
        path_gt, path_target, path_lr = self.filenames[idx]

        wav_gt, sr_nb = ta.load(path_gt)
        wav_target, sr_wb = ta.load(path_target)
        wav_lr, sr_lr = ta.load(path_lr)


        wav_gt = wav_gt.view(1,-1)
        wav_target = wav_target.view(1,-1)
        wav_lr = wav_lr.view(1,-1)

        if self.seg_len > 0 and self.mode == "train":
            duration = int(self.seg_len * 16000)
            sig_len = wav_gt.shape[-1]
            ...
            # Repeat
            wav_gt = wav_gt.repeat(1, t_end // sig_len + 1)[..., t_start:t_end]            
            wav_target = wav_target.repeat(1, t_end // sig_len + 1)[..., t_start:t_end]
            wav_lr = wav_lr.repeat(1, t_end // sig_len + 1)[..., t_start:t_end]

        elif self.mode == "val":
            min_len = min(wav_gt.shape[-1], wav_target.shape[-1], wav_lr.shape[-1])
            wav_gt = self.ensure_length(wav_gt, min_len)
            wav_target = self.ensure_length(wav_target, min_len)
            wav_lr = self.ensure_length(wav_lr, min_len)
            pass

        else:
            sys.exit(f"unsupported mode! (train/val)")

        return wav_gt, wav_target, wav_lr, get_filename(path_gt)[0]
